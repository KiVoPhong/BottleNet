...
print('splitting point:', str(args.div_position) + '_' + str(args.sub_div_position), 'input dim:', args.in_dim,
      'encoded dim', args.hid_dim, 'spatial shrink:', args.spatial)
...
class BottleNetPlusPlus_ResNet(nn.Module):
    def __init__(self, input_channel=args.in_dim, hidden_channel=args.hid_dim, noise=args.noise, channel=args.channel,
                 div_position=args.div_position, spatial=args.spatial):
        super(BottleNetPlusPlus_ResNet, self).__init__()
        self.resnet_model = torch.load('resnet50_0.7781.pkl', map_location='cpu')
        ...
        self.compression_module = compression_module(input_channel=input_channel, hidden_channel=hidden_channel,
                                                     noise=noise, channel=channel, spatial=spatial)
...
def test(epoch):
    with torch.no_grad():
        ...
        if (100 * correct / total) > 60:
            ...
            torch.save(model, args.channel + '_div:' + str(args.div_position) + '_sub_div:' + str(
                args.sub_div_position) + '_spatial:_' + str(args.spatial) + '_hid:' + str(
                args.hid_dim) + '_noise:' + str(args.noise) + '_acc{:.4f}_top5:{:.4f}_'.format((100 * correct / total),
                                                                                               (
                                                                                                           100 * correct_top5 / total)) + 'epoch:' + str(
                epoch) + '.pkl')
        print('Accuracy of the network on the 10000 test images: {} %'.format(100 * correct / total),
              'top5: {} %'.format(100 * correct_top5 / total))
