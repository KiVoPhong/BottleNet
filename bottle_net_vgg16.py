...
print('splitting point:', str(args.div_position), 'input dim:', args.in_dim, 'encoded dim', args.hid_dim,
      'spatial shrink:', args.spatial)
...
class BottleNetPlusPlus_VGG(nn.Module):
    def __init__(self, input_channel=args.in_dim, hidden_channel=args.hid_dim, noise=args.noise, channel=args.channel,
        ...
        # self.vgg_model =torch.hub.load('pytorch/vision:v0.6.0', 'vgg11', pretrained=True)
...
def test(epoch):
    with torch.no_grad():
        ...
        if (100 * correct / total) > 60:
            ...
            torch.save(model, args.channel + '_div:' + str(args.div_position) + '_spatial:_' + str(
                args.spatial) + '_hid:' + str(args.hid_dim) + '_noise:' + str(
                args.noise) + '_acc{:.4f}_top5:{:.4f}_'.format((100 * correct / total),
